...
extension), and runs that example's scene. Add `--no-run` (or use
`minc build`) to compile without launching.
...
`minc run mygame`. The build script picks up any `*.mc` in this folder
automatically.
